package ru.snapix.library.settings
...
fun configurationOptions(block: ConfigurationOptionsBuilder.() -> Unit): ConfigurationOptions {
    val config = ConfigurationOptionsBuilder()
    config.apply(block)
    return config.build()
}

class ConfigurationOptionsBuilder {
